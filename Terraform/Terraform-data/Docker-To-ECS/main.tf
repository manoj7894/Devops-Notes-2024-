...
resource "null_resource" "build_and_push" {
  ...
  provisioner "local-exec" {
    command = "docker build -t my-docker-repo:latest ."
  }

  provisioner "local-exec" {
    command = "aws ecr get-login-password --region ${var.availability_zone} | docker login --username AWS --password-stdin ${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.availability_zone}.amazonaws.com"
  }

  provisioner "local-exec" {
    command = "docker tag my-docker-repo:latest your-account-id.dkr.ecr.your-region.amazonaws.com/my-docker-repo:latest"
  }

  provisioner "local-exec" {
    command = "docker push your-account-id.dkr.ecr.your-region.amazonaws.com/my-docker-repo:latest"
  }
}
